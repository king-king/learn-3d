...
function simulate() {
    ...
    if ( jellyfishTargets.count < Param.jCount ) {
        jellyfishTargets[ jellyfishTargets.count ] = new JellyfishTarget(
            Math.random() * 2 * Param.pBbox[ 0 ] - Param.pBbox[ 0 ] ,
            Math.random() * 2 * Param.pBbox[ 1 ] - Param.pBbox[ 1 ] - 40 ,
            Math.random() * 2 * Param.pBbox[ 2 ] - Param.pBbox[ 2 ] ,
            Math.random() * Param.jScaleRandom + Param.jScale ,
    ...
    for ( var i = 0; i < jellyfishTargets.count; i++ ) {
        ...
        for ( var j = 0; j < jellyfishTargets.count; j++ ) {
            if ( i != j ) {
                ...
                dist = V3.length( delta );
                // - (jellyfishTargets[i].scl+jellyfishTargets[j].scl)*6;
